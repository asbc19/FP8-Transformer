...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    # Modification to include no quantization
    if args.quant_data_type.lower() == 'fp32':
        print('Run Original Model')
    
    else:
        from mpemu import mpt_emu
        list_exempt_layers = []
        list_layers_output_fused = []
        if args.quant_data_type.lower() == 'e5m2':
            list_exempt_layers += ["qa_outputs"]
            list_layers_output_fused += [
                "bert.encoder.layer.0.attention.self.attn_scores_matmul", 
                "bert.encoder.layer.1.attention.self.attn_scores_matmul", 
                "bert.encoder.layer.2.attention.self.attn_scores_matmul", 
                "bert.encoder.layer.3.attention.self.attn_scores_matmul", 
                "bert.encoder.layer.4.attention.self.attn_scores_matmul", 
                "bert.encoder.layer.5.attention.self.attn_scores_matmul", 
                "bert.encoder.layer.6.attention.self.attn_scores_matmul", 
                "bert.encoder.layer.7.attention.self.attn_scores_matmul", 
                "bert.encoder.layer.8.attention.self.attn_scores_matmul", 
                "bert.encoder.layer.9.attention.self.attn_scores_matmul", 
                "bert.encoder.layer.10.attention.self.attn_scores_matmul", 
                "bert.encoder.layer.11.attention.self.attn_scores_matmul", 
                "bert.encoder.layer.12.attention.self.attn_scores_matmul", 
                "bert.encoder.layer.13.attention.self.attn_scores_matmul", 
                "bert.encoder.layer.14.attention.self.attn_scores_matmul", 
                "bert.encoder.layer.15.attention.self.attn_scores_matmul", 
                "bert.encoder.layer.16.attention.self.attn_scores_matmul", 
                "bert.encoder.layer.17.attention.self.attn_scores_matmul", 
                "bert.encoder.layer.18.attention.self.attn_scores_matmul", 
                "bert.encoder.layer.19.attention.self.attn_scores_matmul", 
                "bert.encoder.layer.20.attention.self.attn_scores_matmul", 
                "bert.encoder.layer.21.attention.self.attn_scores_matmul", 
                "bert.encoder.layer.22.attention.self.attn_scores_matmul", 
                "bert.encoder.layer.23.attention.self.attn_scores_matmul", 
                ]

        print("Preparing the {} model for {} quantization".format(args.model_type.lower(), args.quant_data_type.lower()))
        print("List of exempt layers : ", list_exempt_layers)
        model, emulator = mpt_emu.quantize_model (model, dtype=args.quant_data_type.lower(), hw_patch='None',
                            list_exempt_layers=list_exempt_layers, list_layers_output_fused=list_layers_output_fused,
                            device=args.device, verbose=True)#args.verbose_logging)
    print(model)
